...
def validate_grouped_sum(filename: str, df: pd.DataFrame, column_name: str):
    if column_name in df.columns:
        assert round(df[column_name].sum(), 2) == round(df[column_name].sum(),
                                                        2), (f"{filename}, {column_name} Grouped data not summed "
                                                             f"correctly.")
        ...
        if not file_name.startswith('.') and os.path.isfile(os.path.join(path, file_name)):
            ...
            with open(os.path.join(path, file_name), 'r') as f:
                ...
                output_file_path = os.path.join(path,
                                                f"/Users/huangp/Documents/Data_team/DT_project/Clean_MMM/Output/{time}_{region}_CLIENT_{media_name}.csv")
